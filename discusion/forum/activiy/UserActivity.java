...
//			if ((user1.getUserRole() != UserRole.USER) && !Objects.equals(user1.getUsername(), user.getUsername())) {
...
public class UserActivity {
	...
	public void upvoteReply(User user) throws NumberFormatException, IOException {
		System.out.println("Enter reply number you want to upvote : ");
		this.upvoteService.addUpvote(this.getReply(), user);
	}
	...
	private Reply getReply() throws NumberFormatException, IOException {
		while(true) {
			Reply reply = this.replyService.getReply(Long.parseLong(Utility.inputFromUser()));
			if (reply != null) {
				return reply;
			}
